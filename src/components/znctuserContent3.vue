<template>
  <el-container class="container">
    ...
    <el-main class="timu">
      <header>
        <div class="yxzj"><span>已选知识点({{checkList.length}}个)</span> <i class="el-icon-delete"
...
export default {
  ...
  methods: {
    // 改变页数查询数据库
    changePage (page) {
      this.$get('changepage2.php', { page: page, row: 8 }).then(
        response => {
          this.title = []
    ...
    xztype () {
      ...
      if (this.xzstatus) {
        ...
        if (this.tktype !== "" && this.selectData !== "") {
          this.$get('timuzongshu2.php', { type: 1, select: this.selectData, difficult1: 1, difficult2: 2, difficult3: 3 }).then(
            response => {
    ...
    tktype () {
      ...
      if (this.tkstatus) {
        ...
        if (this.tktype !== "" && this.selectData !== "") {
          this.$get('timuzongshu2.php', { type: 2, select: this.selectData, difficult1: 1, difficult2: 2, difficult3: 3 }).then(
            response => {
              if (response.data != 404) {
                new Promise(
                  resolve => {
    ...
    jdtype () {
      ...
      if (this.jdstatus) {
        ...
        if (this.tktype !== "" && this.selectData !== "") {
          this.$get('timuzongshu2.php', { type: 3, select: this.selectData, difficult1: 1, difficult2: 2, difficult3: 3 }).then(
            response => {
              if (response.data != 404) {
                new Promise(
                  resolve => {
    ...
    chujuan (type, num) {
      this.loading = true
      this.$get('zsdchujuan.php', { id: type, xz: num[1], tk: num[2], jd: num[3] }).then(
        res => {
          this.loading = false
...
// 设置左边知识点被选中的框
.zhishidian .el-checkbox__input.is-checked .el-checkbox__inner {
  background-color: $color;
  border-color: $color;
}
// 设置分页被选中的框
.zhishidian .el-pagination.is-background .el-pager li:not(.disabled).active {
  background-color: #364044;
}
//被选中时字体的颜色
.zhishidian .el-checkbox__input.is-checked + .el-checkbox__label {
  color: $color;
}
//设置分页框的颜色
.zhishidian .el-pagination.is-background .el-pager li {
  background-color: $color;
  color: white;
}
//设置多选框的颜色
.zhishidian .el-checkbox__inner {
